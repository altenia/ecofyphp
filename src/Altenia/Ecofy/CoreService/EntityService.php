 ...
 */
class EntityService extends BaseDataService {
	 ...
	 * @return Response
	 */
	public function listEntities($criteria, $sortParams = array(), $offset = 0, $limit=100)
	{
	 ...
	 * @return Response
	 */
	public function paginateEntities($queryParams, $page_size = 20)
	{
	    // @TODO: pending
		$query = \Entity::query();
	 ...
	 */
	public function countEntities($criteria)
	{
	...
	{
		$validator = Entity::validator($data);
        if ($validator->passes()) {
            $record = new Entity();
            $record->fill($data);
	 ...
	 */
	public function updateEntity($pk, $data)
	{
		$validator = Entity::validator($data);
        if ($validator->passes()) {
	 ...
	 */
	public function destroyEntity($pk)
	{
